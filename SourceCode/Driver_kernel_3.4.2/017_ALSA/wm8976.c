 ...
 */
static const u16 wm8976_reg1[WM8976_CACHEREGNUM] = {
...
{
    if ((pin != SCLK) && (pin != SDIN) && (pin != CSB))
    {
        printk(" wm8976_control_pin_set pin error:%d \n", pin);
        return -1;
    }
...
static void wm8976_write_data(unsigned int reg, unsigned int data)
{
    ...
    */
    ...
    tmp = (reg<< 9) | (data & 0x1FF);

    if ((reg == WM8976_HPVOLL) || (reg == WM8976_HPVOLR) || (reg == WM8976_INPPGA))
        tmp |= (1<<8);
...
static int wm8976_soc_suspend(struct snd_soc_codec *codec)
{
    wm8976_set_bias_level(codec, SND_SOC_BIAS_OFF);
    return 0;
}

static int wm8976_soc_resume(struct snd_soc_codec *codec)
{
	int i;
	u8 data[2];
	u16 *cache = codec->reg_cache;
...
/* power down chip */
static int wm8976_soc_remove(struct snd_soc_codec *codec)
{
    wm8976_set_bias_level(codec, SND_SOC_BIAS_OFF);
...
static int wm8976_soc_probe(struct snd_soc_codec *codec)
{
	...
	if (ret)
	{
        printk("wm8976_soc_probe add controls fail \n");
        return ret;
	}
	return 0;
}
...
static struct snd_soc_codec_driver soc_codec_dev_wm8976 = {
	...
	.remove =       wm8976_soc_remove,
	.suspend =      wm8976_soc_suspend,
	.resume =       wm8976_soc_resume,
...
static int __devinit wm8976_codec_probe(struct platform_device *pdev)
{
    gpbcon = ioremap(0x56000010, 4);
    if (!gpbcon)
    {
        iounmap(gpbcon);
        printk("wm8976_soc_probe ioremap fail \n");
        return -1;
    }
...
static int __devexit wm8976_codec_remove(struct platform_device *pdev)
{
...
static struct platform_driver wm8976_codec_driver = {
	.driver = {
		.name = "wm8976-codec",
		.owner = THIS_MODULE,
	},
...
module_platform_driver(wm8976_codec_driver);
