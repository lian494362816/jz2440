...
struct device *key_irq_class_dev = NULL;
...
int key_irq_init(void)
{
    ...
    /* /dev/key_irqs */
    key_irq_class_dev = device_create(key_irq_class, NULL, MKDEV(key_irq_major, 0), NULL, KEY_IRQ_CLASS_DEVICE_NAME);
...
{
    device_destroy(key_irq_class, MKDEV(key_irq_major, 0));
    class_unregister(key_irq_class);
