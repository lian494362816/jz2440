...
static struct device *key_poll_class_dev = NULL;
...
static ssize_t key_poll_read(struct file *file, char __user *userbuf, size_t count, loff_t *off)
{
...
int key_poll_init(void)
{
    ...
    key_poll_class_dev = device_create(key_poll_class, NULL, MKDEV(key_poll_major, 0), NULL, KEY_POLL_CLASS_DEV_NAME);
...
{
    device_destroy(key_poll_class, MKDEV(key_poll_major, 0));
