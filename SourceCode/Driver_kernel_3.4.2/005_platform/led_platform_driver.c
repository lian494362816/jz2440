...
static struct device *led_drv_class_dev = NULL;
...
static int led_driver_probe(struct platform_device *pdev)
{
    ...
    led_drv_class_dev = device_create(led_drv_class, NULL, MKDEV(led_drv_major, 0), NULL, LED_DRV_CLASS_DEV_NAME);
...
err_free_class_dev:
    device_destroy(led_drv_class, MKDEV(led_drv_major, 0));
