...
//return 1, boot start from nand flash
//return 0, boot start from nor flash
static int is_nand_flash_start(void)
{
...
void copy2sdram(void)
{
    ...
    //must change to int, if not, the value will reduce 4 times
    len = ((int)&__bss_start) - ((int)&__copy_code_start);

    if (is_nand_flash_start())
    {
        nand_flash_init();

        nand_flash_read(src, dst, len);
    }
    else
    {
        while(dst < end)
