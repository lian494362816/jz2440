...
static struct class_device *reg_editor_class_device = NULL;
...
static int reg_editor_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
    int ret = 0;
...
static int reg_editor_init(void)
{
    ...
    reg_editor_class_device = class_device_create(reg_editor_class, NULL, dev, NULL, REG_EDITOR_CLASS_DEVICE_NAME);
...
static void reg_editor_exit(void)
{
    ...
    class_device_destroy(reg_editor_class, dev);
