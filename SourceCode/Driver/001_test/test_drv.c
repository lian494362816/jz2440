...
static struct class_dev *test_class_dev[CLASS_DEVICE_MAX];
...
int my_module_init(void)
{
    ...
    {
        test_class_dev[i] = class_device_create(test_class, NULL, MKDEV(MODULE_MAJOR, 0), NULL, "test%d", i);
...
err_free_class:
    for (i -= 1; i >= 0; i--)
    {
        class_device_unregister(test_class_dev[i]);
    }
