	...
	do {
        ...
        */
		...
		if (id_val == DM9KS_ID || id_val == DM9010_ID) {
			...
			PRINT_INFO("chip_revisio:0x%x \n", db->chip_revision);

			chip_info = ior(db,0x43);
			if((db->chip_revision!=0x1A) || ((chip_info&(1<<5))!=0) || ((chip_info&(1<<2))!=1))
			{
                PRINT_ERR("chip_revision and chip_info not find \n");
			    //return -ENODEV;
            }
			/* driver system function */
			dev->base_addr 		= iobase;
			dev->irq 		= irq;
			dev->open 		= &dmfe_open;//open函数会注册中断
...
static volatile unsigned long *BWSCON = NULL;
static volatile unsigned long *BANKCON4 = NULL;
/* Description:
   when user used insmod to add module, system invoked init_module()
   to initilize and register.
*/
int __init dm9kc_init(void)
{
    irq = IRQ_EINT7;
    ...
    /*
    [17:16] Determine data bus width for bank 4.
    ...
    *BWSCON |= (0x1 << 16);
    ...
    /*
    2440                                   DM9K
    Tcas:地址数据要稳定多久后才可选中芯片；T1:命令和片选可以同时发出；Tacs = 0
    Tcos:片选信后多久后才可以读写；        T1:命令和片选可以同时发出; Tcos = 0
    Tacc:读写所需的时间;                   T2：最小10ns; Tacc>=10ns
    Tcho:读写完后，片选信号还要保持多久；  T4：系统数据还需要保持3ns,T6两次读之间的间隔>=2ns； Tcho>=3ns
    Tcah:片选取消后，地址数据还要保持多久；T5: 片选信号和地址可以同时取消；  Tach=0
    Tacp:片选模式，不使用片选模式；
    PMC:00 通常模式；
    */
    *BANKCON4 = 0;
...
module_init(dm9kc_init);
module_exit(dm9kc_exit);
